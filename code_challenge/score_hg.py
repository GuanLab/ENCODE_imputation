...
path1='./avocado/'
# chromimpute
path2='./chromimpute/CHALLENGE/OUTPUTDATA1/'
...
num_metric = 12
array_score = np.zeros((len(list_out), num_metric, len(chr_all) + 1))

for the_assay in assay_all:
    dict_var={}
    for the_chr in chr_all:
        dict_var[the_chr]=np.load('../data_challenge/var_final/var_' + the_assay + '_' + the_chr + '.npy')
    for the_cell in dict_assay_cell[the_assay]:
        ...
        for k,the_chr in enumerate(chr_all):
            print(the_chr)
            ## load data
            mat=np.zeros((len(list_out)+1, chr_len25[the_chr]))
            # 0.gt
            ...
            x=np.concatenate((x,tmp))
            x=x.reshape((-1,25)).T
            mat[1,:]=np.mean(x,axis=0)
            # 2.chromimpute
            mat[2,:]=np.loadtxt(path2 + the_chr + '_impute_' + 'C' + the_cell + '_' + the_assay + '.wig', skiprows=2)
            ...
            x=np.concatenate((x,tmp))
            x=x.reshape((-1,25)).T
            mat[3,:]=np.mean(x,axis=0)

            ## order for extra metrics
            mat_argsort=np.zeros((len(list_out)+1, chr_len25[the_chr]),dtype=int)
            mat_rank=np.zeros((len(list_out)+1, chr_len25[the_chr]))
            ...
            for i in range(1,mat.shape[0]):
                ...
                array_score[i-1,:,k] = [mseglobal, pear, spear, mseprom, msegene, mseenh, msevar, mse1obs, mse1imp, match1, catch1obs, catch1imp]
        # weighted average score based on the chromosome lengths
        for i in range(len(list_out)):
            for j in range(num_metric):
                array_score[i,j,-1] = np.sum(weight * array_score[i,j,:-1])
            list_out[i].write('%s\t' % the_id)
            list_out[i].write('%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\n' % tuple(array_score[i,:,-1]))
            list_out[i].flush()
